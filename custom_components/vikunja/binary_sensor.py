...
def get_binary_sensors_for_task(base_url, task: Task):
    return [
        VikunjaTaskDoneSensor(base_url, task),
    ]
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for task in tasks:
        ...
        entities.extend(get_binary_sensors_for_task(vikunja_api.base_url, task))

    if not entities:
        LOGGER.warning("No entities created")

    async_add_entities(entities, True)
    LOGGER.info(f"Added {len(entities)} Vikunja binary sensors.")
